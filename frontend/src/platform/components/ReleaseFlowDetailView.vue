...
const agentHomePath = computed(() => `/wwa/${props.agentKey}`)
...
async function openViewResult(task: Task) {
  ...
  try {
    const result = await props.api.getTaskResult(task.id, task.latestExecutionId)
...
async function handleStartDeployment(request: Request) {
  ...
  try {
    await props.api.startRequestDeployment(request.releaseFlowId, request.id)
    await store.refreshDetail()
  } catch {
    // Error handled by axios interceptor
  } finally {
...
async function handleMarkRequestFailed(request: Request) {
  ...
  try {
    await props.api.markRequestFailed(request.releaseFlowId, request.id)
    await store.refreshDetail()
  } catch {
    // Error handled by axios interceptor
  } finally {
...
function archiveRundownConfirmationMessage(request: Request): string {
  const activeRequestCount = store.detail?.requests.filter((item: Request) => !item.archivedAt).length ?? 0
  if (activeRequestCount <= 1) {
    return props.copy.archiveLastStageMessage(request.stage)
  }
  return props.copy.archiveNonLastStageMessage(request.stage)
}
...
async function handleArchiveRundown(request: Request) {
  ...
  try {
    const result = await props.api.archiveRequestRundown(request.releaseFlowId, request.id)
    if (result.releaseFlowArchived && !includeArchivedView.value) {
      await store.fetchList()
      await router.push(agentHomePath.value)
      return
    }
    await store.refreshDetail()
  } catch {
...
async function handleRestoreRundown(request: Request) {
  ...
  try {
    await props.api.restoreRequestRundown(request.releaseFlowId, request.id)
    await store.fetchList()
    await store.refreshDetail()
  } catch {
    // Error handled by axios interceptor
...
function purgeRundownConfirmationMessage(request: Request): string {
  ...
  if (totalRequestCount <= 1) {
    return props.copy.purgeLastStageMessage(request.stage)
  }
  return props.copy.purgeNonLastStageMessage(request.stage)
}
...
async function handlePurgeRundown(request: Request) {
  ...
  try {
    const result = await props.api.purgeRequestRundown(request.releaseFlowId, request.id)
    await store.fetchList()
...
const activeRequestSummary = computed(() => {
  ...
  const completedTaskCount = request.tasks.filter((task: Task) =>
    ['Approved', 'Rejected', 'Skipped', 'Completed', 'Failed'].includes(task.taskStatus),
  ).length
  const dependencyEdges = request.tasks.reduce(
    (sum: number, task: Task) => sum + parseDependencyList(task.dependencies).length,
    0,
  )
  const tasksWithDependencies = request.tasks.filter(
    (task: Task) => parseDependencyList(task.dependencies).length > 0,
  ).length
  const unresolvedDependencyCount = request.tasks.reduce((sum: number, task: Task) => {
  ...
  const plannedStarts = request.tasks
    .map((task: Task) => task.plannedStartTime)
    .filter((time: string | undefined): time is string => !!time)
    .map((time: string) => new Date(time))
    .filter((date: Date) => !Number.isNaN(date.getTime()))

  const plannedEnds = request.tasks
    .map((task: Task) => task.plannedEndTime)
    .filter((time: string | undefined): time is string => !!time)
    .map((time: string) => new Date(time))
    .filter((date: Date) => !Number.isNaN(date.getTime()))
  ...
  const plannedStart =
    plannedStarts.length > 0
      ? new Date(Math.min(...plannedStarts.map((date: Date) => date.getTime())))
      : null
  const plannedEnd =
    plannedEnds.length > 0
      ? new Date(Math.max(...plannedEnds.map((date: Date) => date.getTime())))
      : null
...
<template>
  <div class="detail-view">
    <!-- Back navigation -->
    <div class="nav-back">
      <button class="btn btn-secondary btn-sm" @click="router.push(agentHomePath)">
        ← Back
      </button>
    </div>
    ...
    <template v-else-if="store.detail">
          ...
          <div v-if="activeRequestSummary" class="stage-rundown">
            ...
            <div class="rundown-section">
              <div class="rundown-request-actions">
                ...
                >
                  {{ requestActionLoadingId === `${req.id}:start` ? 'Starting...' : copy.startButtonLabel }}
                </button>
                <button
                  type="button"
      ...
      <div v-else class="empty-state">{{ copy.emptyRequestsLabel }}</div>
    </template>
    ...
    <DecisionDialog
      ...
      :submit-decision-fn="api.submitDecision"
    ...
    <RundownEditDialog
      ...
      :update-request-rundown-fn="api.updateRequestRundown"
