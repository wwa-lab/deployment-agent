...
watch(
  () => store.detail,
  (detail) => {
    if (!detail || detail.requests.length === 0) {
      activeTab.value = 0
      return
    }
    const firstActive = detail.requests.findIndex((request) => !request.archivedAt)
    activeTab.value = firstActive >= 0 ? firstActive : 0
  },
)
...
<template>
  <div class="detail-view">
    <div class="nav-back">
      <button class="btn btn-secondary btn-sm" @click="router.push('/wwa/build-agent')">
        ← Back
      </button>
    </div>
    ...
    <div v-if="store.loading && !store.detail" class="loading-state">
      ...
      <span>Loading build workflow...</span>
    </div>
        ...
        >
          <div v-if="activeRequestSummary" class="stage-rundown">
            <div class="rundown-head">
              <div class="rundown-title">Build Rundown</div>
            ...
            <div v-if="req.archivedAt" class="rundown-section archived-rundown-note">
              This build rundown is archived. Task history remains visible, but task changes stay disabled in this view.
            </div>
            ...
            <div class="rundown-section">
              ...
              <div class="rundown-progress-grid">
                <div class="rundown-progress-block">
                  ...
                  <div class="rundown-progress-value">{{ activeRequestSummary.progressPercent }}%</div>
                </div>
                <div class="rundown-progress-block">
                  <div class="rundown-progress-label">Pending Review</div>
                  <div class="rundown-progress-value">{{ activeRequestSummary.pendingReviewCount }}</div>
                </div>
                <div class="rundown-progress-block">
                ...
                <div class="rundown-progress-block">
                  <div class="rundown-progress-label">Task Groups</div>
                  <div class="rundown-progress-value">{{ activeRequestSummary.taskGroupCount }}</div>
                </div>
                <div class="rundown-progress-block">
              ...
              <div class="task-dependency-head">
                <div>
                  ...
                  <div class="task-dependency-copy">
                    Build Agent uses the same task dependency semantics as the other agent workspaces, but remains scoped to DEV-only execution.
                  </div>
                </div>
              </div>
          ...
          <template v-else>
            ...
            <div v-if="viewingResult.result.externalJobUrl" class="external-link-section">
              ...
              <div class="external-link-row">
                ...
                <span
                  v-if="viewingResult.result.submissionStatus"
                  class="badge"
                  :class="viewingResult.result.submissionStatus === 'SUBMITTED' ? 'badge-completed' : 'badge-failed'"
                >
                  {{ viewingResult.result.submissionStatus }}
                </span>
              </div>
              <div
                v-if="viewingResult.result.submissionMessage && viewingResult.result.submissionStatus === 'FAILED'"
    ...
    <TaskActivityDialog
      ...
      :list-task-executions-fn="listBuildTaskExecutions"
