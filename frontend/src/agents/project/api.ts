import { projectClient } from './index'
import type {
...
export async function listReleaseFlows(
  ...
  const response = await projectClient.get('/release-flows', { params })
...
export async function getReleaseFlow(
  ...
  const response = await projectClient.get(`/release-flows/${id}`, { params })
  return response.data
}
...
export async function createRundownFromTemplate(
  ...
  const response = await projectClient.post('/release-flows/from-template', input)
  return response.data
}
...
export async function updateRequestRundown(
  ...
  const response = await projectClient.patch(
...
export async function startRequestDeployment(flowId: string, requestId: string): Promise<Request> {
  const response = await projectClient.post(`/release-flows/${flowId}/requests/${requestId}/start`)
  return response.data
}

export async function markRequestFailed(flowId: string, requestId: string): Promise<Request> {
  const response = await projectClient.post(`/release-flows/${flowId}/requests/${requestId}/fail`)
  return response.data
}
...
export async function purgeRequestRundown(
  ...
  const response = await projectClient.delete(
...
export async function listTaskExecutions(taskId: string): Promise<TaskExecutionHistory[]> {
  const response = await projectClient.get(`/tasks/${taskId}/executions`)
...
export async function recordResult(
  ...
  const response = await projectClient.post(`/tasks/${taskId}/record-result`, body)
  return response.data
}

export async function submitDecision(taskId: string, decision: string): Promise<Task> {
  const response = await projectClient.post(`/tasks/${taskId}/decision`, {
...
export async function editTask(
  ...
  const response = await projectClient.put(`/tasks/${taskId}/input`, inputParameters)
  return response.data
}
...
export async function editNames(
  ...
  const response = await projectClient.put(`/tasks/${taskId}/names`, names)
  return response.data
}
...
export async function editExecutionType(
  ...
  const response = await projectClient.put(`/tasks/${taskId}/execution-type`, { executionType })
  return response.data
}

export async function cloneTask(taskId: string): Promise<Task> {
  const response = await projectClient.post(`/tasks/${taskId}/clone`)
  return response.data
}

export async function reorderTasks(requestId: string, taskIds: string[]): Promise<Task[]> {
  const response = await projectClient.put('/tasks/reorder', { requestId, taskIds })
  return response.data
}
...
export async function submitAutoExecution(taskId: string): Promise<Task> {
  const response = await projectClient.post(`/tasks/${taskId}/submit-auto`)
  return response.data
}

export async function startManualExecution(taskId: string): Promise<Task> {
  const response = await projectClient.post(`/tasks/${taskId}/start-manual`)
  return response.data
}
...
export async function uploadFile(
  ...
  const response = await projectClient.post('/upload', formData, {
    headers: {
...
export async function downloadTemplate(): Promise<Blob> {
  const response = await projectClient.get('/upload/template', {
    responseType: 'blob',
  })
  return response.data
}
