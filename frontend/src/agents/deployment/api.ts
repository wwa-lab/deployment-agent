import { deploymentClient } from './index'
import type {
...
export async function listReleaseFlows(
  ...
  const response = await deploymentClient.get('/release-flows', { params })
...
export async function getReleaseFlow(
  ...
  const response = await deploymentClient.get(`/release-flows/${id}`, { params })
  return response.data
}
...
export async function createRundownFromTemplate(
  ...
  const response = await deploymentClient.post('/release-flows/from-template', input)
  return response.data
}
...
export async function updateRequestRundown(
  ...
  const response = await deploymentClient.patch(
...
export async function listTaskExecutions(taskId: string): Promise<TaskExecutionHistory[]> {
  const response = await deploymentClient.get(`/tasks/${taskId}/executions`)
...
export async function recordResult(
  ...
  const response = await deploymentClient.post(`/tasks/${taskId}/record-result`, body)
  return response.data
}

export async function submitDecision(taskId: string, decision: string): Promise<Task> {
  const response = await deploymentClient.post(`/tasks/${taskId}/decision`, {
...
export async function editTask(
  ...
  const response = await deploymentClient.put(`/tasks/${taskId}/input`, inputParameters)
  return response.data
}
...
export async function submitAutoExecution(taskId: string): Promise<Task> {
  const response = await deploymentClient.post(`/tasks/${taskId}/submit-auto`)
  return response.data
}

export async function startManualExecution(taskId: string): Promise<Task> {
  const response = await deploymentClient.post(`/tasks/${taskId}/start-manual`)
  return response.data
}
...
export async function uploadFile(
  ...
  const response = await deploymentClient.post('/upload', formData, {
    headers: {
...
export async function downloadTemplate(): Promise<Blob> {
  const response = await deploymentClient.get('/upload/template', {
    responseType: 'blob',
  })
  return response.data
}
